...
Definition shutdown : NetworkTypes.Socket -> ShutdownCmd -> IO.IO unit :=
  fun s stype =>
...
Definition gracefulClose : NetworkTypes.Socket -> GHC.Num.Int -> IO.IO unit :=
  fun s tmout =>
    let bufSize := #1024 in
    let unregister :=
...
(* External variables:
     ...
     Network.Socket.Buffer.recvBufNoWait
     Network.Socket.Internal.throwSocketErrorIfMinus1Retry_
     Network.Socket.Types.close Network.Socket.Types.withFdSocket NetworkTypes.Socket
