...
Definition socket
   : Network.Socket.Types.Family ->
     ...
     Network.Socket.Types.ProtocolNumber -> IO.IO NetworkTypes.Socket :=
  fun family stype protocol =>
    let unsetIPv6Only :=
      fun s =>
        GHC.Base.when (andb (family GHC.Base.== Network.Socket.Types.AF_INET6)
                            (Data.Foldable.elem stype (cons NetworkTypes.Stream (cons
...
Definition listen : NetworkTypes.Socket -> GHC.Num.Int -> IO.IO unit :=
  fun s backlog =>
...
Definition connect {sa} `{Network.Socket.Types.SocketAddress sa}
   : NetworkTypes.Socket -> sa -> IO.IO unit :=
  fun s sa =>
...
Definition accept {sa} `{Network.Socket.Types.SocketAddress sa}
   : NetworkTypes.Socket -> IO.IO (NetworkTypes.Socket * sa)%type :=
  fun listing_sock =>
    let callAccept :=
      fun fd sa sz =>
...
(* External variables:
     ...
     Network.Socket.Types.ProtocolNumber Network.Socket.Types.SocketAddress
     Network.Socket.Types.SocketType Network.Socket.Types.c_close
     ...
     NetworkTypes.Socket NetworkTypes.Stream
*)
