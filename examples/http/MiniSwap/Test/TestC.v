...
From DeepWeb Require Import
     ...
     Lib.Util
     Lib.LinearSpec_Core.
From DeepWeb.Free.Monad Require Free OpenUnion.
Import OpenUnion.SumNotations.
Open Scope sum_scope.
Import Effect.NonDeterminism.
...
Definition cid_fd := connection_id * file_descr.

Definition find_cf (cid : connection_id) : list cid_fd -> option cid_fd :=
  find (fun cf => (cid = fst cf)?).

Definition find_fd (cid : connection_id) (l : list cid_fd) : option file_descr :=
  snd <$> find_cf cid l.
...
(*
(* Coq should really implement [Import Free (Ret, Tau, Vis).]
   See coq/coq#8356 *)
Definition Ret := Free.Ret.
Definition Tau := Free.Tau.
Definition Vis := Free.Vis.
*)

(*
Definition execute' : itree specE unit -> list byte -> Client real_trace :=
  fixpoint
    (fun execute' itree msgs =>
       match itree with
       | Free.Ret tt => closeAll;; ret []
       | Free.Tau itree => execute' itree msgs
       | Free.Vis _ e k =>
         match e with
         | (failure_e ||) => closeAll;; ret []
         | ((| nondet_e) |) =>
           match nondet_e in nondetE T return (T -> _) -> _ with
           | Or => fun k =>
             b <- lift flip;;
             execute' (k b) msgs
           end k
         | (| observer_e) =>
           match observer_e in observerE T return (T -> _) -> _ with
           | ObsConnect =>
             fun k =>
               ocft <- newConnection;;
               match ocft with
           ...
           | ObsToServer cid =>
             fun k =>
               ...
               | msg::msgs =>
                 connections <- get;;
                 match find_fd cid connections with
                 | Some fd =>
                   ev <- sendMessage cid fd msg;;
                   tr <- execute' (k msg) msgs;;
                   ret (ev::tr)
                 | None =>
                   closeAll;; ret []
                 end
           ...
           | ObsFromServer cid =>
             fun k =>
               connections <- get;;
               match find_cf cid connections with
               | Some (cid, fd) =>
                 ob <- lift (recvb fd);;
                 let oev := FromServer cid <$> ob in
                 let evs :=
                     match oev with
                     | Some ev => [ev]
                     | None    => []
                     end in
                 tr <- execute' (k ob) msgs;;
                 ret (evs ++ tr)
               | None => execute' (k None) msgs
               end
           end k
         end
       end).
...
Definition execute (msgs : list byte) : Client real_trace :=
  ...
  tr <- execute' (length msgs * 20) msgs;;
...
Instance genMoreBytes : GenSized (list byte) :=
  {| arbitrarySized := arbitrarySized ∘ (mult 10) ∘ S |}.
...
Definition execute_prop' (msgs : list byte) : Checker :=
  ...
  | Some tr =>
    ...
    | _ :: _ => whenFail (show tr)
      (is_disordered_trace_test
         Swap_LinearSpec.test_swap_spec tr)
    end
  end%string.

Definition execute_prop : Checker :=
  forAllShrinkNonDet 100 arbitrary shrink execute_prop'.
  (* forAll arbitrary execute_prop'. *)
