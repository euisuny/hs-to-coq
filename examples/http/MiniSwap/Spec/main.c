...
populate_response(connection* conn, store* last_msg_store) {
  ...
  uint8_t* response_buffer = 
    ...
    /*!! response buffer */ /*! conn->request_buffer */;

  memcpy(response_buffer, /*!*/ last_msg
	 /*!! copy from wrong address */ /*! response_buffer */ , last_msg_len);
  ...
  uint8_t* request_buffer = 
    /*!*/ conn->request_buffer
    /*!! request buffer */ /*! conn -> response_buffer */;
  memcpy(last_msg, request_buffer,
	 /*!*/ request_len 
	 /*!! partial copy */ /*! request_len - 1 */);
  return 1;
}
...
conn_read(connection* conn, store* last_msg_store) {
  ...
  uint8_t* ptr = 
    ...
    /*!! recv address */ /*! request_buffer */
    /*!! recv to response buffer */ /*! conn->response_buffer */;
  r = recv(conn_fd, ptr, BUFFER_SIZE - request_len, 0)
    /*!*/ /*!! return value of recv */ /*! + 1 */;
  uint32_t new_len; 
...
process_connections(connection *head, fd_set* rs, fd_set* ws,
  ...
  /*
    ...
    - SOCKAPI st ;
      ...
      That is, there is a list segment starting from what head_ptr points to 
      to what curr_ptr points to (exclusive), and another segment from 
      curr_ptr until a null pointer is reached. 

      Moreover, process_connections refines the interaction tree 
...
int select_loop(socket_fd server_socket, store* last_msg_store) {
  ...
  /*
    ...
    - SOCKAPI st ;
      ...
      That is, head_ptr points to a linked list of connections, and the loop 
      refines the interaction tree [select_loop server_addr ...]. 
    ...
    - Forall (fun '(conn, fd, ptr) => consistent_state BUFFER_SIZE st (conn, fd))
        connections, i.e. all connections in the list are consistent with a 
