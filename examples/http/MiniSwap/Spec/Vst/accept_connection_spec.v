...
From DeepWeb.Spec
     ...
     Vst.MonadExports Swap_ImplModel.
...
Definition accept_connection_spec (T : Type) :=
  ...
  WITH k : option connection -> itree socketE T,
       addr : endpoint_id,
       st : SocketMap,
       fd : sockfd,
  ...
  PRE [ _socket__1 OF tint, 
    ...
    SEP ( SOCKAPI st ;
            ...
            data_at head_sh (tptr (Tstruct _connection noattr))
                    curr_head ptr_to_head ;
            lseg LS Ews Ews elems curr_head nullval
        )
  POST [ tint ]
    ...
    SEP ( SOCKAPI st' ; 
            ...
            | Some (conn, fd, new_head) => 
              ITREE (k (Some conn)) *
              malloc_token Ews (Tstruct _connection noattr) new_head * 
              data_at head_sh (tptr (Tstruct _connection noattr))
