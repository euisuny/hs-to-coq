...
Module SockAPIPred.
  ...
  (* This is used only at the entry of main to initialize socket memory, 
     because VST does not currently consider SOCKAPI as an external resource. 
     When that is added, this axiom can be dropped. 
   *)
  Axiom init_sockapi:
    emp |-- SOCKAPI {| lookup_socket := fun _ : sockfd => ClosedSocket |}.
...
Module TracePred.
  ...
  (* In the paper, [t] is of type [itree implE] instead of [itree socketE]. 
     This is for simplicity. 
     We have a function [simplify_network] that transforms a tree over [socketE]
     to one that is over [implE]. 
   *)
  Definition ITREE {T} (t : itree socketE T) :=
    EX t' : itree socketE T, !!(t <= t')%trace && has_ext t'.

  Lemma trace_pred_incl:
    forall {T : Type} (t1 t2 : itree socketE T),
      (t2 <= t1)%trace ->
      ITREE t1 |-- ITREE t2.
  Proof.
    unfold ITREE.
  ...
  Lemma internal_nondet1: 
    forall {T1 T2 : Type} (k1 k2 : itree socketE T1) (k3 : T1 -> itree socketE T2),
      ITREE (r <- or k1 k2 ;; k3 r) |--
      ITREE (r <- k1 ;; k3 r).
  Proof.
    intros. apply trace_pred_incl.
    epose proof (trace_or_incl_bind k1 k2) as H.
    apply H.
  Qed.
  ...
  Proof.
    intros. apply trace_pred_incl.
    epose proof (trace_or_incl_bind k1) as H.
    apply H.
  Qed.
    
  Lemma internal_nondet3:
    forall {T1 T2 : Type} (k : T1 -> itree socketE T2) (xs : list T1) (x : T1),
      In x xs ->
  ...
  Proof.
    ...
    epose proof (trace_choose_incl x xs k).
    auto.
  Qed.

  Lemma trace_bind_ret :
    forall {A B} a (f : A -> itree socketE B),
  ...
  Proof.
    ...
    split; apply trace_incl_eutt;
      [symmetry |]; apply Utt.push_tau; reflexivity.
  Qed.
  
  Lemma trace_bind_assoc:
    forall {A B C : Type} (m : itree socketE A) (f : A -> itree socketE B)
      (g : B -> itree socketE C),
      ITREE ( b <- (a <- m ;; f a) ;; g b ) =
      ITREE ( a <- m ;; b <- f a ;; g b ).
  Proof.
    ...
    split;
      ...
      [ | symmetry ]; apply Utt.bind_bind.
  Qed.
  
  Lemma trace_drop_tau:
    forall (T : Type) (k : itree socketE T), 
      ITREE (Tau k) = ITREE k.
  Proof.
    intros.
    apply trace_pred_eq.
  ...
  Lemma trace_bind_cancel:
    forall {A B : Type} (m : itree socketE A) (f g : itree socketE B),
      eq_utt f g ->
      ITREE ( m ;; f ) = ITREE ( m ;; g ).
  Proof.
    intros.
    ...
    apply trace_eq_incl2.
    simpl.
    split; apply trace_incl_eutt; apply bind_mor;
      [ reflexivity
      | intro; assumption
      | reflexivity
...
Module FDSetPred.
  ...
  (* This axiom posits an opaque representation of an FD_Set Coq record
     in memory.
   *)
  ...
  (* This axiom says that there is an abstract Coq record that corresponds 
     to the contents of the fd_set in memory. 
   *)
...
Definition SIZE_MAX : Z := two_p 16 - 1.
...
Definition accept_spec (T : Type) :=
  DECLARE _accept
  WITH t : itree socketE T,
       k : connection_id -> itree socketE T,
...
Definition send_spec (T : Type) :=
  ...
  PRE [ 1%positive OF tint, 2%positive OF (tptr tvoid),
    ...
    PROP ( consistent_world st;
           lookup_socket st fd = ConnectedSocket client_conn;
           (length (val_of_string msg) <= Z.to_nat SIZE_MAX)%nat;
           readable_share sh; 
           forall l, (0 <= l <= length (val_of_string msg))%nat ->
                trace_incl
...
Definition recv_spec (T : Type) :=
  DECLARE _recv
  WITH t : itree socketE T,
       k : option string -> itree socketE T,
       client_conn : connection_id,
       st : SocketMap,
       fd: sockfd,
       buf_ptr: val,
  ...
  POST [ tint ]
    ...
    PROP ( (0 <= r <= alloc_len) \/ r = NO;
             ...
             (exists msg, result = inr (Some msg) /\
                     length (val_of_string msg) = Z.to_nat r /\
             ...
             r < 0 -> (result = inl tt /\
                      ...
                      st' = st)
             ;
             length contents = Z.to_nat alloc_len;
             consistent_world st'
         )
    LOCAL ( temp ret_temp (Vint (Int.repr r)) )
    SEP ( SOCKAPI st' ;
