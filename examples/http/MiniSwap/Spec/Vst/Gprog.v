From VST
     Require Import sepcomp.extspec veric.juicy_mem veric.semax_ext veric.semax_prog.
...
Definition socket_ext_spec
  ...
  add_funspecs_rec
    ext_link
    (ok_void_spec (itree socketE unit)).(@OK_ty)
    (ok_void_spec (itree socketE unit)).(@OK_spec)
    socket_specs.
...
(*
Definition main_spec (tree : itree socketE unit) :=
  DECLARE _main
  WITH gv : globals
  PRE [ ]
  (PROP ( )
...
Definition main_spec (tree : itree socketE unit) :=
  DECLARE _main main_spec_ext' prog real_swap_impl
  (fun _ _ => PROP ( False )
  LOCAL ( temp ret_temp (Vint (Int.repr 0)) )
  SEP ( )).
...
(* This axiom states that the wrapper for the fd_zero macro 
   (called fd_zero_macro in main.c) satisfies the specification of fd_zero.
 *)
Axiom body_fd_zero_macro:
  semax_body Vprog Gprog f_fd_zero_macro fd_zero_macro_spec.

(* This axiom states that the wrapper for the fd_set macro 
   (called fd_set_macro in main.c) satisfies the specification of fd_set.
 *)
Axiom body_fd_set_macro:
  semax_body Vprog Gprog f_fd_set_macro fd_set_macro_spec.

(* This axiom states that the wrapper for the fd_isset macro 
   (called fd_isset_macro in main.c) satisfies the specification of fd_isset.
 *)
