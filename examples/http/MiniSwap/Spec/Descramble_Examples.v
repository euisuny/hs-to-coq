...
Require Import DeepWeb.Lib.Util DeepWeb.Lib.LinearSpec.

Require Import DeepWeb.Spec.Swap_LinearSpec.
...
(* "Scrambled traces" describe what the clients _across the network_
   ...
   The [is_disordered_trace_test] function checks whether a given
   "observed trace" is a disordered version of a trace belonging
   to a given specification.  If so, it returns the "reordered
   trace" that explains the observed trace. *)

(* Every actual trace of the server is also a disordered trace: *)
Example disordered_trace_example_1 :
  is_disordered_trace_test test_swap_spec [
    0 !;
    1 !;
    0 <-- "a";
    0 <-- "b";
...
Example bad_disordered_trace_example_1 :
  is_disordered_trace_test test_swap_spec [
    0 !;
    0 <-- "a";
    0 <-- "b";
...
(* A more interesting bad trace, where the server appears to send the
   ...
   vice versa_. *)
Example bad_disordered_trace_example_2 :
  is_disordered_trace_test test_swap_spec [
    0 !;
    1 !;
    2 !;
...
Example disordered_trace_example_3 :
  is_disordered_trace_test test_swap_spec [
    0 !;
    1 !;
    0 <-- "a";
...
(* A variant where things are even more interleaved: *)
Example disordered_trace_example_3' :
  is_disordered_trace_test test_swap_spec [
    0 !;
    1 !;
...
(* The reason for including these "hypothetical events" in
   ...
   clearly impossible: *)
Example bad_disordered_trace_example_3 :
  is_disordered_trace_test test_swap_spec [
    0 !;
    1 !;
    0 <-- "a";
...
(** * Network refinement examples *)

Definition def {C} (f : _ -> _ -> C) : C :=
  f (ServerDefs.SwapConfig.req_size ServerDefs.test_swap_config)
    (ServerDefs.SwapConfig.initial_state ServerDefs.test_swap_config).
...
Definition test_swap := network_refines_test
                          test_swap_spec
...
CoFixpoint eager_server (tt : unit) : itree implE unit :=
  c <- accept ;;
  eager_server tt.

Definition test_eager := network_refines_test
                           test_swap_spec
                           (eager_server tt).
...
CoFixpoint const_server (buffer_size : nat) : itree implE unit :=
  c <- accept ;;
  msg <- recv c buffer_size ;;
  send c "0"%string;;
...
CoFixpoint echo (tt : unit) : itree implE unit :=
  c <- accept ;;
  msg <- recv c 2 ;;
  send c msg ;;
  echo tt.

CoFixpoint echo_spec (tt : unit) : itree specE unit :=
  c <- obs_connect ;;
  msg <- obs_msg_to_server 2 c;;
  obs_msg_from_server c msg ;;
  echo_spec tt.

Definition test_echo := network_refines_test
                          (echo_spec tt)
                          (echo tt).
