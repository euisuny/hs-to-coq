...
(* Effect types can be combined using the [+'] operator:
   ...
   [embed : (I -> E X) -> (I -> itree S X)]
   [embed : (I -> J -> E X) -> (I -> J -> itree S X)]
   ...
   [fail : forall E `{failureE -< E}, string -> itree E S]
...
Definition or {E} `{nondetE -< E} {X} (k1 k2 : itree E X)
  : itree E X :=
...
Definition get `{stateE -< E} : itree E S := embed Get.
Definition put `{stateE -< E} : S -> itree E unit := embed Put.
End StateSection.
...
Definition ask {E} `{readerE -< E} : itree E R :=
  liftE (convert Ask).

(* Given a value [r] we can remove all [Ask] nodes, passing
   [r] to each continuation. *)
CoFixpoint run_reader {E A} (r : R) (m : itree (E +' readerE) A)
  : itree E A :=
  match m with
  | Ret a => Ret a
  | Vis (| e ) k =>
...
Definition tell `{Convertible writerE E} (w : W) : itree E unit :=
