...
Definition alt `{HasAlt E} {X} (m1 m2 : itree E X) : itree E X :=
...
Definition commit `{Convertible commitE E} : itree E unit :=
...
Definition PureSpec := itree specE.
...
Variable zipE : forall {X Y}, E X -> CE Y -> itree F (X * Y).

CoFixpoint zipM {A B} (p : itree (F +' E) A) (t : itree (F +' CE) B)
  : itree F (A + B) :=
  match p, t with
    | Vis (| e ) pk, Vis (| ce ) tk =>
        p <- zipE e ce;;
    ...
    | Vis ( e |) pk, _ => Vis e (fun x => zipM (pk x) t)
    | Ret a, _ => Ret (inl a)
    | _, Tau t' => Tau (zipM p t')
    | _, Vis ( e |) tk => Vis e (fun x => zipM p (tk x))
    | _, Ret b => Ret (inr b)
  end.
