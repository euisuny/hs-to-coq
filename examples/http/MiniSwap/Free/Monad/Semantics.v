...
Lemma stable_assoc E A B C :
  forall S (pre : forall X, E X -> S -> Prop)
         ...
         (post : S -> C -> Prop)
         (s : S)
         (m : itree E A) (k : A -> itree E B) (h : B -> itree E C),
    Stable _ _ pre step post s (m >>= (fun a => k a >>= h)) ->
    Stable _ _ pre step post s ((m >>= k) >>= h).
Proof.
...
Lemma stable_map_assoc E A B C :
  forall S (pre : forall X, E X -> S -> Prop)
         ...
         (post : S -> C -> Prop)
         (s : S)
         (m : itree E A) (f : A -> B) (h : B -> itree E C),
    Stable _ _ pre step post s (m >>= (fun a => h (f a))) ->
    Stable _ _ pre step post s (map_itree f m >>= h).
Proof.
  intros S pre step post.
  cofix self.
  intros.
...
Lemma stable_map_itree E A B :
  forall S (pre : forall X, E X -> S -> Prop)
    ...
    forall (s : S) (m : itree E A),
      Stable _ _ pre step postA s m ->
      Stable _ _ pre step postB s (map_itree f m).
Proof.
  intros S pre step postA postB f Hf.
  cofix self.
  intros.
  inversion H; rewrite match_itree; simpl.
  - apply StableRet.
    apply Hf; assumption.
  - apply StableTau.
...
Lemma hoist_def E F X Y (e : F Y) (k : Y -> itree F X)
      (f : forall X, F X -> E X) :
  hoist f (Vis e k) = Vis (f _ e) (fun y => hoist f (k y)).
Proof.
  rewrite (match_itree (hoist _ _)).
  simpl.
  auto.
Qed.

Lemma hom_def E F X Y (e : F Y) (k : Y -> itree F X)
      (interpret : forall Y, F Y -> itree E Y) :
  hom interpret _ (Vis e k)
  =
  interpret _ e >>= fun y =>
    hom interpret _ (k y).
Proof.
  rewrite (match_itree (hom _ _ _)).
  rewrite (match_itree (bind_itree _ _)).
  simpl.
  auto.
Qed.
...
Lemma stable_run E F S_ S
      ...
      (step : forall X, E X -> S -> S -> X -> Prop)
      (refn : S_ -> S -> Prop)
      (interpret : forall X, F X -> itree E X)
      X
      (post_ : S_ -> X -> Prop)
      (post : S -> X -> Prop) :
  (forall Y (e : F Y) s_ s,
...
Proof.
  ...
  inversion Hm.
  - rewrite match_itree; simpl; apply StableRet.
    eapply Hpost.
    eassumption.
    assumption.
  - rewrite match_itree; simpl.
    apply StableTau.
