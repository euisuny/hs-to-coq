...
Fixpoint MtoT {E X} (fuel : nat) (m : itree (labeledE E) X) : T E X :=
  match fuel with
  | O => (...)
  | S fuel' =>
    match m with
...
CoInductive ImpM {E} {X} `{Visibility E} : relation (itree E X) :=
...
CoInductive SimM {E} {X} `{Visibility E} : relation (itree E X) :=
