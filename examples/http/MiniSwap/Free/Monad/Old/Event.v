...
(* [RunTo p t q]: Program-state [p] executes with trace [t]
   until state [q].
   [F]: invisible events (e.g., nondeterminism)
   [E]: visible events *)
Inductive RunTo {E F X}
  : itree (F +' E) X -> event_trace E -> itree (F +' E) X -> Prop :=
| Stop : forall p, RunTo p [] p
| Step : forall Y (e : E Y) (y : Y) (k : Y -> itree (F +' E) X) t q,
    RunTo (k y) t q ->
    RunTo (Vis (| e) k) (Event e y :: t) q
| Work : forall p t q, RunTo p t q -> RunTo (Tau p) t q
| Guess : forall Y (e : F Y) (y : Y) (k : Y -> itree (F +' E) X) t q,
    RunTo (k y) t q ->
    RunTo (Vis (e |) k) t q.

(* Preorder (if we ignore the trace index). *)
Lemma RunTo_refl {E F X} : forall (p : itree (F +' E) X), RunTo p [] p.
Proof. constructor. Qed.

Lemma RunTo_trans {E F X} : forall (p q r : itree (F +' E) X) t u,
...
Lemma RunTo_trans' {E F X} : forall (p q r : itree (F +' E) X) t u tu,
...
CoInductive Simulate {E F X} : itree (F +' E) X -> itree (F +' E) X -> Prop :=
...
Definition TraceIncl {E F X} (p q : itree (F +' E) X) : Prop :=
...
Lemma Simulate_TraceIncl {E F X} : forall (p0 q0 : itree (F +' E) X),
...
CoInductive PreSimulate {E F X A B}
  : itree (F +' E) X -> itree (F +' E) A -> (B -> itree (F +' E) A) -> Prop :=
| PreSimRet : forall (x : X) (b : B) (r : B -> itree (F +' E) A),
...
CoInductive Stable {F A}
  : Inv.I -> itree (F +' failureE +' Inv.E) A -> Prop :=
| StableRet : forall i (a : A), Stable i (Ret a)
| StableTau : forall i m, Stable i m -> Stable i (Tau m)
| StableInvis :
    forall i Y (e : F Y) (k : Y -> itree _ A),
      (forall y, Stable i (k y)) ->
      Stable i (Vis (e||) k)
| StableVis :
    forall i Y (e : Inv.E Y) (k : Y -> itree _ A),
      Inv.pre _ e i ->
      (forall s s' i' y, Inv.step _ e s i s' i' y -> Stable i' (k y)) ->
      Stable i (Vis (|e) k).

CoInductive Stable' {F A}
  : Inv.J -> itree (F +' failureE +' Inv.E) A -> Prop :=
...
| Stable'Invis :
    forall i Y (e : F Y) (k : Y -> itree _ A),
      ...
      Stable' i (Vis (e||) k)
| Stable'Vis :
    forall j Y (e : Inv.E Y) (k : Y -> itree _ A),
      Inv.pre' _ e j ->
      (forall s s' j' y, Inv.step' _ e s j s' j' y -> Stable' j' (k y)) ->
      Stable' j (Vis (|e) k).
