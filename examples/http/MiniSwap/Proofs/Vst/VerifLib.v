...
From DeepWeb.Proofs.Vst Require Export
     VerifLibTraces.
...
Definition malloc_tokens (t : type) (ptrs : list val) :=
  fold_right (fun ptr a => sepcon a (malloc_token Ews t ptr)) emp ptrs.

Lemma cons_malloc_tokens (t : type) (ptrs : list val) (ptr : val):
  malloc_tokens t ptrs * malloc_token Ews t ptr
...
Module Trace_Tactics.
  ...
  Ltac lift_bind_itree_to_bind :=
    repeat
      (match goal with
       | |- context[bind_itree ?m ?k] =>
         change (bind_itree m k)
           with (@bind _ (Monad_M _) _ _ m k)
       end).
    ...
    | [|- context[ITREE ?t]] =>
      ...
      | (r <- ?or_trace ;; ?k) =>
        ...
        | or ?t1 ?t2 =>
          ...
          assert (Traces.trace_incl (r <- t1 ;; k) t)
            as HTrace
              by (apply trace_or_incl_bind1)
        end
      | or ?t1 ?t2 =>
        remember t1 as left_trace;
        assert (Traces.trace_incl t1 t) as HTrace
  ...
  Ltac intro_trace_refl_incl HTrace :=
    match goal with
    | [|- context[ITREE ?t]] =>
      assert (Traces.trace_incl t t) as HTrace
  ...
  Ltac take_branch1 idx := 
    ...
    | [|- context[ITREE (bind (or ?branch1 ?branch2) ?k)]] =>
      ...
      [go_lower; lift_bind_itree_to_bind; apply internal_nondet1 | ]
    end.
  ...
  Ltac take_branch2 idx := 
    ...
    | [|- context[ITREE (bind (or ?branch1 ?branch2) ?k)]] =>
      ...
      [go_lower; lift_bind_itree_to_bind; apply internal_nondet2 | ]
    end.
...
Module Buffer_Bounds.
  ...
  Ltac intro_rep_in_buffer_bound :=
    ...
    [ (* there really should be a better way to do this *)
      match goal with
      | [H: context[@Zlength ?ty ?v] |- Zlength ?v <= _] =>
        change ty with val in H; omega
      end |
    ].
