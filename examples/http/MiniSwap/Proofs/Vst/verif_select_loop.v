...
(* This is the invariant at the head of the main loop (while (1 == 1) { ... } ) in 
   main.c. See comments in main.c for an explanation. 
...
Definition main_loop_invar
           ...
           (v_ws : val)
           (v_es : val)
           (msg_store_ptr : val) (msg_store_sh : share) : environ -> mpred :=
  ...
  SEP ( SOCKAPI st ;
        ...
        ITREE (r <- select_loop real_swap_app server_addr
                 (map proj_conn connections, last_msg)
               ;; k);
...
Proof.
  ...
  set (main_loop_inv :=
         main_loop_invar k server_addr server_fd
                         ...
                         v_rs v_ws v_es msg_store_ptr msg_store_sh).
  ...
  { (* while loop *)
    ...
    *)    
    ...
    | [|- context[LOCALx (?Locs) (SEPx (_ :: ?Frame)) ]] =>
      forward_if
        (EX conn_opt : option (connection * sockfd * val),
         ...
         (SEPx (
              SOCKAPI st' ::
              lseg LS Ews Ews (map rep_full_conn connections')
              head_ptr' nullval ::
    ...
    post_accept connections st0 head_ptr v_head
                (fun connections' =>
                   select_loop real_swap_app server_addr
                               (map proj_conn connections', last_msg)
                               ;; k).
    ...
    (Forall (fun '(conn, fd, _) =>
               consistent_state real_swap_app st1 (conn, fd)) connections1)
      as consistent_at_st1.
    {
      ...
      - econstructor; auto.
        + simpl App.req_size.
          ...
          unfold SIZE_MAX.
          rewrite Z2Nat.id by (simpl; omega).
          simpl; omega.
        + simpl.
          omega.
      ...
      as NoDup_ids_connection1.
    {
        ...
        as lookup_conn'_fd_st1.
      {
        ...
        apply (consistent_RECVING_SENDING_connected real_swap_app);
          try assumption.
    ...
    forward_call (k, st1, server_addr, server_fd, connections1,
                  ...
                  v_rs, v_ws, msg_store_ptr, msg_store_sh).
