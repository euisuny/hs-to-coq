...
(* This is the invariant for the loop in process_connections in main.c. See 
   comments in main.c for an explanation. 
 *)
...
Definition process_loop_prop_invar
  ...
  [consistent_world st;
     Forall (fun '(conn, fd, ptr) =>
               consistent_state real_swap_app st (conn, fd)) prefix;
     Forall (fun '(conn, fd, ptr) =>
               consistent_state real_swap_app st (conn, fd)) suffix;
     exists old_prefix,
  ...
  :=
    [SOCKAPI st ;
     ITREE ( (select_loop
                real_swap_app
                server_addr
                (map proj_conn (prefix ++ suffix), last_msg))
               ;; k );
     FD_SET Tsh read_set read_set_ptr;
     FD_SET Tsh write_set write_set_ptr;
     lseg LS lst_sh lst_sh (map rep_full_conn prefix) head_ptr curr_ptr;
     lseg LS lst_sh lst_sh (map rep_full_conn suffix) curr_ptr nullval;
...
Definition process_loop_invar
  ...
  (SEPx (process_loop_sep_invar
           ...
           msg_store_ptr msg_store_sh prefix suffix lst_sh
           st last_msg curr_ptr
  ))).
...
Proof.
  ...
  { (* body of while *)
    ...
    assert (consistent_state real_swap_app st0 (conn, fd)) as Hconsistent.
    { 
      match goal with
    ...
    | context[list_cell _ ?lst_sh (rep_connection ?conn ?fd) ?ptr] =>
    match process_pred with 
    | context[SOCKAPI ?st] => 
    match process_pred with 
    | context[field_at ?msg_store_sh (Tstruct _store noattr) _
      ...
      (EX conn' : connection,
       ...
       (SEPx ( SOCKAPI st' ::
               ...
               list_cell LS lst_sh (rep_connection conn' fd) ptr ::
    ...
    { (* if (socket_ready > 0) *)
      ...
      unfold select_loop.
      rewrite loop_with_state_unfold.
      simpl.

      lift_bind_itree_to_bind.
      ...
      forward_call (after_process, st0, conn, fd,
                    last_msg0, ptr, Ews, msg_store_ptr, msg_store_sh).
      { unify_tree.
        cancel.
      }
      ...
      assert
        (Forall (fun '(conn0, fd0, _) =>
                   consistent_state real_swap_app st' (conn0, fd0)) prefix
         /\ Forall (fun '(conn0, fd0, _) =>
                     consistent_state real_swap_app st' (conn0, fd0)) rest)
        as frame_consistent.
      {
      ...
      assert (conn_id conn = conn_id conn') as conn_id_preserved.
      {
        ...
        - eapply recv_step_preserves_conn_ids with (app := real_swap_app);
            eauto.
      ...
      
      {
        ...
        - repeat split; auto.
          ...
          + match goal with
            ...
            symmetry.
            eapply
              recv_step_preserves_frame_lookup with (app := real_swap_app);
              eauto.
