...
Ltac forward_send fd buf_ptr :=
  ...
  simpl_trace_incl HTrace;
  match goal with
  | [H: (bind (send_any_prefix ?client_conn ?msg) ?k <= ?t)%trace |- _] =>
    match goal with
    | [|- context[SOCKAPI ?st]] =>
      match goal with
      | [|- context[data_at ?sh _ _ buf_ptr]] =>
...
Ltac forward_accept fd :=
  ...
  simpl_trace_incl HTrace;
  match goal with
  | [H:  (bind (accept ?server_addr) ?k <= ?t)%trace |- _] =>
    match goal with
    | [|- context[SOCKAPI ?st]] =>
...
Ltac forward_shutdown fd :=
  ...
  simpl_trace_incl HTrace;
  match goal with
  | [H: (((shutdown ?conn_id) ;; ?k) <= ?t)%trace |- _] =>
    match goal with
    | [|- context[SOCKAPI ?st]] =>
...
Ltac forward_listen fd backlog :=
  ...
  simpl_trace_incl HTrace;
  match goal with
  | [H: (bind (listen ?addr) (fun _ => ?k) <= ?t)%trace |- _] =>
    match goal with
    | [|- context[SOCKAPI ?st]] =>
