(* Traces that a client observes are disordered ("descrambled")
   by the network.
   We try to explain a disordered trace by "reordering" it into
   a trace that is recognized by a given spec. *)
...
Require Import DeepWeb.Lib.LinearSpec_Observer.
...
(* We enumerate disorderings in a tree structure, using
   [nondetE] to branch, so that each successful path (i.e., not
   leading to failure) is a reordering of a given trace. *)
Definition eventE' := failureE +' nondetE +' eventE.

(* Helper for [pick_event]. *)
CoFixpoint pick_event' (t_prev t : real_trace) : itree eventE' real_trace :=
  match t with
  | [] => fail "empty trace"
  | ev :: t =>
    let pick_this :=
...
Fixpoint list_eventE' (fuel : nat) (s : itree eventE' unit)
...
Definition list_eventE (fuel : nat) (s : itree eventE' unit) :
  option (list hypo_trace) :=
...
(* [s]: tree of acceptable "linear" traces (spec)
   [t]: disordered trace

   The result tree has a [Ret] leaf iff there is a reordered
   trace accepted by [s] ([is_trace_of]).
 *)
CoFixpoint intersect_trace
            (s : itree specE unit)
            (t : real_trace) :
  itree (failureE +' nondetE +' eventE) unit :=
  match s with
  ...
  | Vis ( e |) k => Vis ( e |) (fun x => intersect_trace (k x) t)
  | Vis (| e ) k =>
    match filter is_FromServer t with
    | [] => ret tt
    | _ :: _ =>
...
CoFixpoint find' (ts : list (hypo_trace * itree eventE' unit)) :
  itree emptyE (option hypo_trace) :=
  match ts with
  | [] => ret None
  | (tr, t) :: ts =>
    match t with
...
Fixpoint to_result (fuel : nat) (m : itree emptyE (option hypo_trace)) :
  ...
  | S fuel =>
    ...
    | @Vis _ _ X e k => match e in emptyE X' with end
    end
  end.

Definition is_disordered_trace_test_
           (fuel : nat) (s : itree specE unit) (t : real_trace) :
  result hypo_trace unit :=
  to_result fuel (find' [([], intersect_trace s t)]).

Definition is_disordered_trace_test :
           itree specE unit -> real_trace -> result hypo_trace unit :=
  is_disordered_trace_test_ (5 * _1000).

(* We will then generate traces produced by a server to test them
   with [is_disordered_trace_test].
   There are two ways:
   ...
   - We can generate traces by walking through the itree model of
     the C program. [Lib.LinearSpec_ServerTrace]
 *)
