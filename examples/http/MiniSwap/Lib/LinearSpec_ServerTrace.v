...
(** [shuffle xs] generates a permutation of [xs].  *)
Definition shuffle {A} (xs : list A) : G (list A) :=
  let n := length xs in
  (fix shuf n xs :=
    match n, xs with
    | S n, x :: xs' =>
      zys <- elems_ (x, xs) (picks xs);;
      let '(z, ys) := zys in
      zs <- shuf n ys;;
      ret (z :: zs)
    | _, _ => ret []
...
Definition traceM :=
  itree (failureE +' nondetE +' arbitraryE +' note_traceE).
...
Definition enum_traces (t : itree implE unit) : traceM unit :=
  map_itree snd (hom_state enum_traces_handler _ (0, []) t).

(**)

(* We can now define simple random generators from the trees produced by
   [enum_traces]. This is useful for debugging and as a small extra example.
   However for actual testing it seems more efficient to have a more stateful
   generator that can backtrack; see following section. *)
...
Definition network_refines_test_
           (max_depth : nat)
           (backtrack_fuel : nat)
           (reorder_fuel : nat)
           (observer : itree specE unit)
           (server : itree implE unit) :=
  let check_trace := is_disordered_trace_test_ reorder_fuel observer in
...
Definition network_refines_test
           (observer : itree specE unit)
           (server : itree implE unit) :=
  network_refines_test_ _1000 _100 (5 * _1000) observer server.
